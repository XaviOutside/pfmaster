...
describe('IStorage type-level contract', () => {
  ...
  it('must accept calls with default parameters (page, limit, clientId, petId)', async () => {
    ...
    const storage: IStorage = {
      ...
      getClient: (_id: number) => Promise.resolve({} as Client),
      createClient: (_data: CreateClientDto) => Promise.resolve({} as Client),
      updateClient: (_id: number, _data: UpdateClientDto) => Promise.resolve({} as Client),
      deleteClient: (_id: number) => Promise.resolve(),
      reactivateClient: (_id: number) => Promise.resolve({} as Client),
      deactivateClient: (_id: number) => Promise.resolve({} as Client),
      searchClients: (_query: string) => Promise.resolve([] as Client[]),
      listPets: () => Promise.resolve(emptyPage as PaginatedResponse<Pet>),
      getPet: (_id: number) => Promise.resolve({} as Pet),
      createPet: (_data: CreatePetInput) => Promise.resolve({} as Pet),
      updatePet: (_id: number, _data: UpdatePetInput) => Promise.resolve({} as Pet),
      deletePet: (_id: number) => Promise.resolve(),
      deactivatePet: (_id: number) => Promise.resolve({} as Pet),
      searchPets: (_query: string) => Promise.resolve([] as Pet[]),
      listServices: () => Promise.resolve(emptyPage as PaginatedResponse<Service>),
      getService: (_id: number) => Promise.resolve({} as Service),
      createService: (_data: CreateServiceInput) => Promise.resolve({} as Service),
      updateService: (_id: number, _data: UpdateServiceInput) => Promise.resolve({} as Service),
      deleteService: (_id: number) => Promise.resolve(),
      deactivateService: (_id: number) => Promise.resolve({} as Service),
      searchServices: (_query: string) => Promise.resolve([] as Service[]),
      listAppointments: (_start: string, _end: string) => Promise.resolve([] as Appointment[]),
      getAppointment: (_id: number) => Promise.resolve({} as Appointment),
      createAppointment: (_data: CreateAppointmentDto) => Promise.resolve({} as Appointment),
      updateAppointment: (_id: number, _data: UpdateAppointmentDto) => Promise.resolve({} as Appointment),
      cancelAppointment: (_id: number) => Promise.resolve({} as Appointment),
      getSettings: () => Promise.resolve({} as CompanySettings),
      updateSettings: (_data: UpdateSettingsDto) => Promise.resolve({} as CompanySettings),
      uploadLogo: (_file: File) => Promise.resolve({} as CompanySettings),
    };
